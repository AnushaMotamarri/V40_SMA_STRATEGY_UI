...
import { downloadJSONAsCSV, getCSVFilename } from '../../utils/utils';
...
function ResearchReports() {
  ...
  const columnConfigs = [
    ...
    { accessor: "opportunity_variation", label: "Opp Variation %" },
  ...
  function handleGenerateReports(data){
    setLoading(true);
    setResult([])
    fetch("http://localhost:8000/research-stock", {
      method: "POST",
      headers: {
  ...
  return (
    <div>
        ...
        <h2>Results</h2>
        <span className='ml-auto display-flex fs-md '><a className='flex-center' onClick={()=>downloadJSONAsCSV(result,getCSVFilename('research'))}><DownloadIcon style={{fontSize:'13px'}}/> Download </a></span>
        </div>
